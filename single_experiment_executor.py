...
def run_aggs(es, thread_number, shared_state_for_threads):
    ...
    while True:
        if shared_state_for_threads['continue_running_aggs']:
            ...
            result=es.search(index=global_vars.HIGH_HIGH_CARDINALITY_INDEX, doc_type='doc', body=request)
...
def configure_index_and_run_background_inserts(es, experiment_obj, shared_state_for_threads):
    ...
    request_body = {
        'settings': global_vars.RESULT_INDEX_SETTINGS,
        'mappings': global_vars.RESULT_INDICES_MAPPINGS
    }
    ...
    es.indices.put_settings(index=global_vars.HIGH_HIGH_CARDINALITY_INDEX, body=new_index_settings)
    es.indices.put_mapping(doc_type='doc', index=global_vars.HIGH_HIGH_CARDINALITY_INDEX, body=new_index_mappings)
